...
| **MITRE ATT&CK v14** | strong and broader | 42 mapped skills across cloud, identity, Kubernetes, container, MCP, and remediation paths, including the shipped AWS IAM login-profile slice and the AWS / GCP / Azure logging-impairment trio |
...
- The AWS role-session slice stays in `detect-lateral-movement`, while
  `detect-aws-login-profile-creation` now covers the first IAM-user
  console-persistence path. Access-key and temporary-credential pivots remain
  tracked separately so the docs do not overstate provider depth.
