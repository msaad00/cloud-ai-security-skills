...
- **`ingestion/`**: 25 ingest skills plus 4 source adapters
...
**Total shipped: 134 skill bundles.** Auto-generated per-framework rollup in [`docs/FRAMEWORK_COVERAGE.md`](docs/FRAMEWORK_COVERAGE.md); per-skill registry in [`docs/framework-coverage.json`](docs/framework-coverage.json).
...
Shipped: optional per-skill `mcp_tool_schema.json` overlays merged into `tools/list` input schemas. Typed-schema skills (14): `ingest-cloudtrail-ocsf`, `ingest-mcp-proxy-ocsf`, `ingest-okta-system-log-ocsf`, `ingest-entra-directory-audit-ocsf`, `ingest-k8s-audit-ocsf`, `detect-mcp-tool-drift`, `detect-prompt-injection-mcp-proxy`, `detect-aws-access-key-creation`, `detect-lateral-movement`, `cspm-aws-cis-benchmark`, `cspm-gcp-cis-benchmark`, `cspm-azure-cis-benchmark`, `convert-ocsf-to-sarif`, `convert-ocsf-to-mermaid-attack-flow`. Validated by `scripts/validate_mcp_tool_schemas.py`.
