...
| **Kubernetes workload quarantine** (`remediate-container-escape-k8s`) | Low — network isolation reversible by deleting the policy | Single pod / workload in one namespace | `human_required` with **declared incident window** + dry-run default | 1 | 0 but dry-run first | Protected namespaces (`kube-system`, `kube-public`, `istio-system`, `linkerd*`) are denied in code. `--reverify` must prove the deny-all policy still exists and still matches the expected selector. |
