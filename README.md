...
<p align="center"><strong>98 production-grade security skills for cloud and AI — OCSF on the wire, MCP-ready, HITL-audited, sandboxed, runs everywhere the same bundle can.</strong></p>
...
**98 shipped skill bundles** — atomic, deterministic, single-concern. Twelve are guarded write paths; the other 86 are read-only. Drop one into a pipeline, an agent, a Step Function, or a `python ... | python ...` one-liner.

| Layer | Count | Purpose | Output |
|---|---:|---|---|
| **Ingest** | 16 | normalize raw cloud / identity / K8s / MCP / GitHub signal | OCSF 1.8 (native opt-in) |
| **Discover** | 5 | inventory · graph · AI BOM · evidence · IAM-departure planning | native / bridge JSON |
| **Detect** | 46 | deterministic rules tagged with MITRE ATT&CK / ATLAS / OWASP | OCSF Detection Finding 2004 |
...
**Total: 98 shipped skills.**  Live counts and per-framework coverage in [`docs/COVERAGE_SNAPSHOT.md`](docs/COVERAGE_SNAPSHOT.md) (auto-generated, CI-gated).
...
**Which vendor signals normalize to OCSF today?** [`docs/INGEST_COVERAGE.md`](docs/INGEST_COVERAGE.md) — the canonical vendor × source × OCSF class matrix, 17 mappings shipped (AWS · GCP · Azure · Entra · K8s · Okta · Workspace · MCP · GitHub) plus the 8 documented roadmap rows (Slack, Workday, Salesforce, SAP, AWS Config, native ClickHouse audit, AWS web-app exfil pipeline, Workspace beyond-login).
...
![Closed-loop coverage matrix — 13 of 46 shipped detections are closed loops today; lateral movement is intentionally detection-only, and the AWS access-key, AWS login-profile, AWS discovery-burst, AWS cross-account S3 copy, AWS/GCP/Azure logging-impairment, AWS/GCP model-artifact download, GCP service-account-key creation, GCP service-account-token minting, MCP credential-leak, system-prompt-extraction, tool-output-policy-bypass, tool-output-exfiltration-instructions, Snowflake bulk egress, Snowflake share creation, Snowflake account-key creation, Snowflake warehouse resize burst, Snowflake unauthorized grant, Snowflake failed-MFA burst, Snowflake session-policy bypass, Snowflake network-policy disable, Snowflake replication-config change, ClickHouse bulk export, and Databricks token-creation slices are detection-first today.](docs/images/coverage-matrix.svg)
