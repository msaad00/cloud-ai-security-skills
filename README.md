...
![Closed-loop coverage matrix — 13 of 19 shipped detections are closed loops today; lateral movement is intentionally detection-only, and the AWS login-profile, AWS/GCP/Azure logging-impairment, plus MCP credential-leak slices are detection-first today.](docs/images/coverage-matrix.svg)
...
| **Detect** | 19 detectors across MITRE ATT&CK, MITRE ATLAS, and MCP / agent-native signals, including the shipped AWS IAM login-profile slice, the AWS / GCP / Azure logging-impairment trio, and MCP credential-leak detection | deeper identity pivots, discovery, exfiltration, and more MCP / AI-native patterns |
