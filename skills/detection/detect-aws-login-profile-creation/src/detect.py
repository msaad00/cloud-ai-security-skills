"""Detect AWS IAM user login-profile creation via CloudTrail.

Reads OCSF 1.8 API Activity (class 6003) records emitted by
`ingest-cloudtrail-ocsf` from stdin or a file. Fires on successful
`CreateLoginProfile` calls against IAM users and emits an OCSF 1.8 Detection
...
LOGIN_PROFILE_CREATE_OPERATION = "CreateLoginProfile"
...
def _build_native_finding(*, event: dict[str, Any], target_user: str) -> dict[str, Any]:
    ...
    return {
        ...
        "rule": "aws-login-profile-creation",
        "api_operation": LOGIN_PROFILE_CREATE_OPERATION,
...
def _to_ocsf(native: dict[str, Any]) -> dict[str, Any]:
    description = (
        ...
        f"{native['src_ip'] or '<unknown>'}. This creates console credential "
    ...
    return {
        ...
        "metadata": {
            ...
            "labels": ["aws", "iam", "passwords", "persistence"],
        },
        "finding_info": {
            "uid": native["finding_uid"],
            "title": "AWS IAM login profile created",
            "desc": description,
            "types": ["aws-login-profile-creation"],
    ...
    *,
    ...
    for event in events:
        ...
        if producer not in ACCEPTED_PRODUCERS:
            ...
            continue
        if _api_operation(event) != LOGIN_PROFILE_CREATE_OPERATION:
            continue
        ...
        if not target_user:
            emit_stderr_event(
                ...
                message="skipping CreateLoginProfile event with no target IAM username",
            )
            continue
