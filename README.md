...
**79 shipped skill bundles** — atomic, deterministic, single-concern. Twelve are guarded write paths; the other 64 are read-only. Drop one into a pipeline, an agent, a Step Function, or a `python ... | python ...` one-liner.
...
| **Detect** | 32 | deterministic rules tagged with MITRE ATT&CK / ATLAS / OWASP | OCSF Detection Finding 2004 |
