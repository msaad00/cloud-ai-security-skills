...
Detailed architecture, security model, IAM role definitions, and framework
mappings for the AWS IAM departures control plane.
...
## Identity Remediation Scope

This skill remediates **AWS IAM** users and access keys only. Its worker Lambda
runs a 13-step deletion (keys → login → groups → policies → MFA → certs → SSH →
service creds → tag → delete) with per-step tracking on a shared
`RemediationResult` interface.

Other identity providers are handled by their **own dedicated skills** — each
using that provider's native services under the same guardrails and skill
contract, rather than one orchestration stack pretending to fit every provider:

| Identity provider | Skill |
|-------------------|-------|
| AWS IAM users + access keys | `iam-departures-aws` (this skill) |
| GCP service accounts + Workspace users | `iam-departures-gcp` |
| Azure Entra ID users | `iam-departures-azure-entra` |

The reconciler's `HRSource` abstraction and `DepartureRecord` schema are
cloud-agnostic; each provider's worker carries its own remediation logic. (Note:
Snowflake, Databricks, and ClickHouse appear elsewhere in this skill as HR
*data sources* the reconciler reads from — not as remediation targets.)
