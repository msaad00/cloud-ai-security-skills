...
![Simplified AWS-only IAM departures flow for the iam-departures-aws skill. Plan scopes the actionable set from HR sources through the reconciler into an immutable S3 manifest. Orchestrate uses EventBridge, Step Functions, a parser Lambda, and a worker Lambda. Write stays inside AWS IAM through an org-bounded cross-account role and follows the ordered teardown represented in code as 11 worker functions spanning 13 distinct IAM API operations. Audit lands in DynamoDB plus KMS-encrypted S3, and the next run verifies closure or drift. Azure and GCP are shown only as native-stack variants, not as shared workers.](docs/images/iam-departures-aws.svg)
