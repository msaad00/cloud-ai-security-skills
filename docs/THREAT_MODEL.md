...
This maps the high-signal scenarios above to concrete automated tests or an
explicit documented exception. The goal is to make coverage visible, not to
...
- `covered` means there is at least one automated test or validator that directly exercises the control.
- `partial, documented exception` means the repo can verify some of the contract locally, but full assurance depends on runtime/operator behavior outside the repo boundary.
