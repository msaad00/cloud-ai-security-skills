...
![Simplified AWS-only IAM departures workflow. One HR source feeds the reconciler, which applies should_remediate(), rehire, grace-period, and hash-diff logic before writing only actionable rows to a KMS-encrypted S3 manifest. S3 ObjectCreated triggers EventBridge and a Step Function; the parser Lambda revalidates entries and current IAM state, and the worker Lambda assumes an Org-scoped cross-account role guarded by aws:PrincipalOrgID. The worker runs the ordered IAM cleanup path, dual-writes to DynamoDB and KMS-encrypted S3, and optional warehouse ingest-back feeds the next reconciler run. Azure and GCP stay on separate native pipelines, so one worker never crosses clouds.](docs/images/iam-departures-aws.svg)
