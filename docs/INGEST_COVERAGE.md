...
get OCSF out the other side?"** The 19 rows below are the canonical answer at
...
> 17 rows for 16 ingest skills: the Entra, Okta, and GitHub normalizers
> each emit multiple OCSF classes depending on the source event family,
> so they are listed once but produce more than one OCSF class.
